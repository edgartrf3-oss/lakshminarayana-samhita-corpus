    ...
    for lineno,raw in enumerate(text.splitlines(),1):
        ...
        if len(hint)>=3:
            ...
            if ak!=khanda or ach!=chapter:
                flags.append("marker_prefix_context_mismatch"); anomalies.append({"type":"marker_prefix_context_mismatch","khanda":khanda,"chapter":chapter,"line":lineno,"canonical_position":canonical,"hint":list(hint)})
            elif av!=canonical:
                flags.append("strong_anchor_position_mismatch"); anomalies.append({"type":"strong_anchor_position_mismatch","khanda":khanda,"chapter":chapter,"line":lineno,"canonical_position":canonical,"anchor_verse":av,"hint":list(hint)})
        verse_text="\n".join(buffer).strip()
